...
#        pwsh ./scripts/deploy.ps1           # deploys to all projects
#        pwsh ./scripts/deploy.ps1 -Only Compass   # deploys to one project by Name
#        pwsh ./scripts/deploy.ps1 -SkipFunctions  # SQL only
#        pwsh ./scripts/deploy.ps1 -SkipDb         # Edge Functions only
#        pwsh ./scripts/deploy.ps1 -SetSecrets     # also push APP_NAME / FEEDBACK_TO_EMAIL / RESEND_API_KEY
...
try {
  foreach ($p in $targets) {
    ...
    if (-not $SkipDb) {
      if (-not $p.DbUrl) { Write-Error "DbUrl missing for $($p.Name)"; exit 1 }
      Write-Host "→ Pushing SQL migrations..." -ForegroundColor Yellow
      supabase db push --db-url $p.DbUrl --yes
    ...
    Write-Host "✓ $($p.Name) done." -ForegroundColor Green
  }
